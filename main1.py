...
# 我们仍然需要 RandomGenerator，但 Dataset 类我们将重写
...
# ----------------------------------------------------------------------
# --- 新增：自定义 ISIC Dataset 类 (解决 split 报错问题) ---
# ----------------------------------------------------------------------
class ISICDataset(torch.utils.data.Dataset):
    def __init__(self, base_dir, list_dir, split, transform=None):
        ...
        list_file_path = os.path.join(list_dir, split + '.txt')
    ...
    def __getitem__(self, idx):
        ...
        # slice_name 格式示例: isic2016/Train/ISIC_0000000

        # 1. 确定图片路径
        # 尝试 jpg 和 png
        img_path = os.path.join(self.base_dir, slice_name + '.jpg')
        if not os.path.exists(img_path):
            img_path = os.path.join(self.base_dir, slice_name + '.png')
            if not os.path.exists(img_path):
                # 如果还是找不到，尝试 bmp
                img_path = os.path.join(self.base_dir, slice_name + '.bmp')

        # 2. 确定 Mask (Label) 路径
        # 逻辑：将文件夹名 Train 替换为 Train_G，Test 替换为 Test_G
        # 并且文件名可能带有 _Segmentation 后缀

        parts = slice_name.split('/')
        if len(parts) >= 3:
            ds_name = parts[0]  # e.g., isic2016
            folder_name = parts[1]  # e.g., Train
            file_name = parts[2]  # e.g., ISIC_0000000

            mask_folder = folder_name + '_G'  # e.g., Train_G

            # 尝试路径 1: ISIC_0000000_Segmentation.png (常见格式)
            mask_path_1 = os.path.join(self.base_dir, ds_name, mask_folder, file_name + '_Segmentation.png')
            # 尝试路径 2: ISIC_0000000.png (直接同名)
            mask_path_2 = os.path.join(self.base_dir, ds_name, mask_folder, file_name + '.png')

            if os.path.exists(mask_path_1):
                label_path = mask_path_1
            elif os.path.exists(mask_path_2):
                label_path = mask_path_2
            else:
                # 如果都找不到，默认使用 path 1 以便抛出明确的 FileNotFoundError
                label_path = mask_path_1
        else:
            # 备用逻辑
            label_path = img_path.replace('Train', 'Train_G').replace('Test', 'Test_G').replace('.jpg', '.png')

        # 3. 加载图片和 Mask
        try:
            image = Image.open(img_path).convert('RGB')
            label = Image.open(label_path).convert('L')  # Mask 是灰度图
        except FileNotFoundError as e:
            logging.error(f"File not found during loading: {e}")
            # 返回一个全黑的 dummy 数据防止程序崩溃（可选）
        ...
        # 4. 预处理 (归一化等，保持与 Synapse_dataset 一致)
        # 确保 label 是 0/1 (二分类)
        label = label / 255.0
        label = (label > 0.5).astype(np.float32)  # 阈值化
        ...
        sample['case_name'] = self.sample_list[idx].replace('/', '_')  # 将路径作为 case_name
        return sample
...
def plot_metrics(history_metrics, current_metrics_list, epoch, save_path, is_final=False):
    ...
    if current_metrics_list:
        ...
        # 提取当前周期的指标值 (假设只有 Class 1, since num_classes=2 for polyp/isic)
...
def custom_test_single_volume(image_batch, label_batch, net, classes, patch_size, test_save_path=None, case=None,
    ...
    try:
        ...
        for class_idx in range(1, classes):
            ...
            # --- 可视化：仅对第一个类别进行可视化 (息肉/皮肤病变通常只有前景一个类别) ---
...
parser.add_argument('--dataset', type=str,
                    default='isic2016', help='experiment_name (e.g., isic2016)')
...
# ----------------------------------------------------------------------
# --- 新增：ISIC数据集列表生成函数 (替代 create_single_dataset_split) ---
# ----------------------------------------------------------------------

def create_isic_lists(dataset_folder_name, dataset_full_path, dataset_list_dir):
    """
    为ISIC数据集生成训练和测试列表。
    直接扫描 'Train' 和 'Test' 文件夹。
    列表格式为: dataset_folder_name/Train/filename (无扩展名)
    """
    import glob

    logging.info(f"Generating lists for dataset: {dataset_folder_name} from {dataset_full_path}")

    # 路径结构: dataset_full_path / Train / *
    train_dir = os.path.join(dataset_full_path, 'Train')
    test_dir = os.path.join(dataset_full_path, 'Test')

    if not os.path.exists(train_dir):
        logging.error(f"Skipping {dataset_folder_name}: Train directory not found at {train_dir}")
        return [], []
    if not os.path.exists(test_dir):
        logging.error(f"Skipping {dataset_folder_name}: Test directory not found at {test_dir}")
        return [], []

    def get_image_names(directory):
        # 获取所有图像名称 (不带扩展名)
        files_png = glob.glob(os.path.join(directory, '*.png'))
        files_jpg = glob.glob(os.path.join(directory, '*.jpg'))
        files_jpeg = glob.glob(os.path.join(directory, '*.jpeg'))
        files_bmp = glob.glob(os.path.join(directory, '*.bmp'))
        all_files = files_png + files_jpg + files_jpeg + files_bmp
    ...
    # 构造列表文件中的路径
    # 格式: dataset_folder_name/Train/image_name
    # 这样 ISICDataset 使用 base_dir (C:\dz12) + 这个路径就能找到文件
    train_cases = [f"{dataset_folder_name}/Train/{name}" for name in train_images]
    test_cases = [f"{dataset_folder_name}/Test/{name}" for name in test_images]

    if not train_cases:
        logging.warning(f"No training cases found in {train_dir}.")
    if not test_cases:
        logging.warning(f"No testing cases found in {test_dir}.")
    ...
    logging.info(f"List files created for {dataset_folder_name} in {dataset_list_dir}")
...
def inference_during_training(args, net, epoch, snapshot_path, history_metrics):
    ...
    # 注意：这里使用 ISICDataset 替代 Synapse_dataset
    db_config = {
        'Dataset': ISICDataset,
        'volume_path': args.root_path,  # 此时 args.root_path 已经是所有数据集的父目录 (e.g., C:\dz12)
        'list_dir': args.list_dir,  # 此时 args.list_dir 已经是当前数据集的 list 目录
        'num_classes': args.num_classes,
        'z_spacing': 1,  # 假设皮肤病变数据集都是2D图像
    }
    ...
    try:
        ...
        if len(all_metrics) > 0:
            ...
            # 对于二分类 (num_classes=2)，我们只关心 class_idx=0 (即前景)
...
def modified_trainer_synapse(args, model, snapshot_path):
    ...
    **MODIFIED: Testing only starts from epoch 50 onwards.**
    **MODIFIED: Uses ISICDataset instead of Synapse_dataset.**
    """
    # from datasets.dataset_synapse import Synapse_dataset # 不再使用原始 Dataset
    from datasets.dataset_synapse import RandomGenerator
    ...
    # --- FIX: Increase num_workers to 4 for faster data loading ---
    NUM_WORKERS = 4
    # ---------------------------------------------------------------

    # 创建训练数据集 (使用 ISICDataset)
    db_train = ISICDataset(base_dir=args.root_path, list_dir=args.list_dir, split="train",
    ...
    for epoch_num in iterator:
        ...
        # --------------------------------------------------------------------------
        # MODIFICATION: Only perform testing after epoch 50, and then every args.test_interval epochs
        # --------------------------------------------------------------------------
        if (epoch_num + 1) >= 50 and (epoch_num + 1) % args.test_interval == 0:
            ...
            logging.info(f"Skipping testing for {args.dataset} at epoch {epoch_num + 1} (before epoch 50).")
        # --------------------------------------------------------------------------
    ...
    # 最终测试 (无论是否达到50 epoch，总会在训练结束时进行一次最终测试)
...
if __name__ == "__main__":
    ...
    # 路径修正点：指向 C:\dz12
    # ====================================================================
    base_data_root = r"C:\dz12"

    # --- 数据集配置字典 ---
    # 包含所有单个数据集的配置。
    dataset_configs_individual = {
        'isic2016': {
            'dataset_folder_name': 'isic2016',
            'list_dir': os.path.abspath(os.path.join('./lists', 'lists_isic2016')),
            'num_classes': 2,
        },
        'isic2017': {
            'dataset_folder_name': 'isic2017',
            'list_dir': os.path.abspath(os.path.join('./lists', 'lists_isic2017')),
            'num_classes': 2,
        },
        'isic2018': {
            'dataset_folder_name': 'isic2018',
            'list_dir': os.path.abspath(os.path.join('./lists', 'lists_isic2018')),
            'num_classes': 2,
        },
    }
    ...
    for current_dataset_name in ISIC_DATASETS:
        print(f"\n=====================================================================")
        ...
        if not os.path.isdir(actual_dataset_path):
            ...
            logging.error("Please ensure the data is correctly placed in 'C:\\dz12'.")
            continue  # 跳过此数据集并处理下一个

        # --- 执行单个数据集的列表生成 (基于 Train/Test 文件夹) ---
        print(f"--- Generating lists for {current_dataset_name} based on Train/Test folders ---")
        create_isic_lists(current_config['dataset_folder_name'],
                          actual_dataset_path,
                          args.list_dir)
        print("------------------------------------------------------------\n")
        ...
        snapshot_path = snapshot_path + '_lr' + str(args.base_lr).replace('.',
                                                                          '') if args.base_lr != 0.01 else snapshot_path
        ...
        # 使用修改后的训练器
        print(
            f"Starting training for {current_dataset_name} with comprehensive evaluation metrics and visualizations...")
