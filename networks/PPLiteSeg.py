...
class PPLiteSegDecoder(nn.Module):
    """
    基于 PP-LiteSeg 论文思想的轻量级解码器。
    它旨在高效地融合来自 ViT 编码器的多阶段（但可能同分辨率）特征，
    并将其高效融合以生成用于分割头的单个特征图。

    此解码器采用以下设计策略：
    1.  **通道对齐与初步融合 (SAM-like)**：
        对每个编码器特征图应用 `1x1 卷积 + BatchNorm + ReLU`。
        这有助于统一不同阶段特征的通道数，并引入非线性。
        由于 ViT-T 输出的特征图分辨率相同，这里直接将处理后的特征图沿通道维度拼接。
    2.  **特征增强 (FEM-like)**：
        使用一个包含**深度可分离卷积 (Depthwise Separable Convolution)** 的模块
        来高效地处理拼接后的特征。深度可分离卷积通过将标准卷积分解为深度卷积
        （spatial filtering）和逐点卷积（channel-wise projection）两步，
        显著减少了参数量和计算量，是构建轻量级网络的关键。
    3.  **最终输出卷积**：
        最后通过一个 `1x1 卷积` 将融合块的输出通道数调整到 `out_channels`，
        以匹配 `SegmentationHead` 的输入要求。

    该解码器特别适用于像 ViT-Tiny/16 这样的非分层式主干，其输出的“多尺度”特征
    在空间分辨率上是相同的。它提供了一种轻量级且有效的特征融合机制。
    """
    ...
    def __init__(self, encoder_channels, decoder_hidden_dim=256, out_channels=256):
        ...
        # 1. 通道对齐与初步融合 (SAM-like)：对每个编码器阶段的特征图进行 1x1 卷积、BatchNorm 和 ReLU
        ...
        # 2. 特征增强 (FEM-like)：使用深度可分离卷积来高效融合拼接后的特征。
        # 输入通道数是所有投影特征图通道数之和 (len(encoder_channels) * decoder_hidden_dim)
        # 输出通道数是 decoder_hidden_dim
        fused_in_channels = len(encoder_channels) * decoder_hidden_dim
        self.fem_block = nn.Sequential(
            # 深度卷积 (Depthwise Convolution): 空间特征提取，对每个输入通道独立进行卷积
            nn.Conv2d(fused_in_channels, fused_in_channels,
                      kernel_size=3, padding=1, groups=fused_in_channels, bias=False),
            nn.BatchNorm2d(fused_in_channels),
            nn.ReLU(inplace=True),
            # 逐点卷积 (Pointwise Convolution): 通道间特征融合，通过 1x1 卷积组合深度卷积的输出
            nn.Conv2d(fused_in_channels, decoder_hidden_dim, kernel_size=1, bias=False),
            nn.BatchNorm2d(decoder_hidden_dim),
            nn.ReLU(inplace=True)
        )

        # 3. 最终输出卷积层，将通道数调整为 `out_channels`，以匹配 SegmentationHead 的输入
        self.output_conv = nn.Conv2d(decoder_hidden_dim, out_channels, kernel_size=1)

    def forward(self, features):
        # features 是来自编码器的特征图列表，所有特征图具有相同的空间分辨率。
        # 例如：[B, C_0, H_feat, W_feat], [B, C_1, H_feat, W_feat], ...

        processed_features = []
        for i, f in enumerate(features):
            # 1. 应用通道对齐与初步融合模块 (1x1 卷积、BatchNorm 和 ReLU)
        ...
        # 3. 应用深度可分离卷积的特征增强模块
        refined_features = self.fem_block(fused_features)
...
class VisionTransformer(nn.Module):
    ...
    **重要提示：**
    ...
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        # PPLiteSegDecoder 最终输出的分辨率将是 ViT 最小特征图的分辨率 (这里是 14x14)。
        ...
        logger.info(f"Using PPLiteSegDecoder.")
    ...
    def forward(self, x):
        ...
        # 将这些“多尺度”特征传递给 PPLiteSegDecoder
