...
class Block(nn.Module):
    """Med-Adapter Block structure integrated into TransUNet Block interface"""
    ...
    def __init__(self, config, vis):
        ...
        # Note: config.adapter_scale and config.thd should be defined in config if needed
    ...
    def forward(self, x):
        ...
        # Space Adapter branch
        attn_out, weights = self.attn(x)
        x = self.Space_Adapter(attn_out)

        # Optional 3D Depth branch (simplified for 2D inputs if thd=False)
        if self.thd:
            # This part requires specific input shapes, typically used in video/volumetric data
            # Here we keep the logic for compatibility
            xd = self.Depth_Adapter(attn_out)
            x = x + xd

        x = x + h  # Residual connection for Attention part
