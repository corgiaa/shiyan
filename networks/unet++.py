...
class Attention(nn.Module):
    def __init__(self, config, vis):
        ...
        # Modified: Hardcoded dropout rate to 0.3
        self.attn_dropout = Dropout(0.3)  # config.transformer["attention_dropout_rate"]
        self.proj_dropout = Dropout(0.3)  # config.transformer["attention_dropout_rate"]
...
class Mlp(nn.Module):
    def __init__(self, config):
        ...
        # Modified: Hardcoded dropout rate to 0.3
        self.dropout = Dropout(0.3)  # config.transformer["dropout_rate"]
...
# 主要的 VisionTransformer 类，现在使用 UNet++ 作为核心分割模型
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        # 使用UNet++替代原来的Transformer + Decoder
        input_channels = 3
        if hasattr(config, 'n_classes'):
            num_classes = config.n_classes
        elif hasattr(config, '__getitem__') and 'n_classes' in config:
            num_classes = config['n_classes']
        else:
            # 如果config中没有n_classes，使用传入的num_classes
            pass

        self.unet_plus_plus = NestedUNet(
        ...
        # 为了兼容性，保留一些原有的属性
        self.transformer = None  # 设为None，因为我们不再使用transformer
        self.decoder = None  # 设为None，因为我们不再使用原来的decoder
        self.segmentation_head = None  # 设为None，UNet++内部已包含输出头
    ...
    def forward(self, x):
        ...
        # 直接使用UNet++进行分割
        logits = self.unet_plus_plus(x)

        return logits

    def load_from(self, weights):
        """
        为了兼容性保留load_from方法，但对于UNet++来说，
        我们通常从头开始训练或加载专门的UNet++预训练权重
        """
        logger.info("UNet++ model loaded. Note: Original ViT weights are not compatible with UNet++.")
        logger.info("Please train from scratch or load UNet++ specific weights.")
        pass
