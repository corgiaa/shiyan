...
# 保留原始的ViT配置导入，尽管PSPNet结构不会使用其大部分参数
...
class VisionTransformer(nn.Module):
    """
    此类别已重构为实现 segmentation_models_pytorch 的 PSPNet 架构，
    ...
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        # PSPNet 的中间输出通道数，这将作为SegmentationHead的输入通道数。
        # 这里设置为256，表示PSPNet模型会输出256个通道的特征图。
        pspnet_output_channels = 256 # 命名更改以反映PSPNet

        # --- segmentation_models_pytorch PSPNet 参数配置 ---
        ...
        # 编码器深度，对应骨干网络的下采样级别数。
        # resnet50 通常有5个级别。
        encoder_depth = 3 # PSPNet通常在较浅的编码器深度上工作，例如3或4。
                          # 对于resnet50，encoder_depth=3 意味着使用到 resnet block3 的特征。
                          # 如果需要更深的特征，可以设置为5。
        ...
        # !!! 将分割模型从 FPN 更改为 PSPNet (Pyramid Scene Parsing Network) !!!
        self.segmentation_model = smp.PSPNet(
            encoder_name=encoder_name,
            encoder_weights=encoder_weights,
            in_channels=in_channels,
            classes=pspnet_output_channels,  # PSPNet 的中间输出通道数，由SegmentationHead进一步处理
            encoder_depth=encoder_depth,
            # PSPNet 也有 psp_channels 和 drop_rate 参数，但 smp 会为不同的编码器提供合理的默认值。
            # 我们可以选择不显式指定，让库自动处理。
            activation=activation,
        )

        self.segmentation_head = SegmentationHead(
            in_channels=pspnet_output_channels,  # 接收PSPNet的中间特征图
    ...
    def forward(self, x):
        ...
        # 通过 segmentation_models_pytorch PSPNet 提取特征
    ...
    def load_from(self, weights):
        logger.warning(
            "The 'load_from' method is specific to Vision Transformer weights and is not applicable to the PSPNet architecture. Skipping weight loading.")
        pass
