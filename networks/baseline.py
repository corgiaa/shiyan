...
import numpy as np
from torch.nn import Dropout, Softmax, Linear, Conv2d, LayerNorm, Embedding
from torch.nn.modules.utils import _pair
from scipy import ndimage
from einops import rearrange  # This import is present but not used in the provided snippet, keeping it as per original.
...
# ==========================================
# Original Block Module (unchanged, as it's a standard ViT block)
# ==========================================
...
# ==========================================
# Modified Embeddings Module (now pure ViT-like patch embeddings, without ResNet features for skip connections)
# ==========================================
class Embeddings(nn.Module):
    def __init__(self, config, img_size, in_channels=3):
        ...
        # This means each "pixel" of the ResNet output becomes a token.
    ...
    def forward(self, x):
        x_resnet, _ = self.hybrid_model(x)

        x = self.patch_embeddings(x_resnet)
        x = x.flatten(2)
        x = x.transpose(-1, -2)
...
class MaskDecoder(nn.Module):
    def __init__(self, config, image_feature_dim, prompt_embed_dim, num_mask_tokens=1):
        ...
        self.mask_upsampling_layers = nn.Sequential(
            ...
            nn.ConvTranspose2d(prompt_embed_dim // 4, prompt_embed_dim, kernel_size=2, stride=2),  # <--- CHANGED HERE
            nn.ReLU(),
            # Final Conv2d to produce the mask logits, now takes prompt_embed_dim channels as input
            nn.Conv2d(prompt_embed_dim, num_mask_tokens, kernel_size=1)  # <--- CHANGED HERE
        )
        # --- FIX END ---
