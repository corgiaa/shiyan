...
# ==========================================
# 新增创新模块: Multi-Scale Channel-Axial Attention (MCAA)
# ==========================================
class MCAA(nn.Module):
    """
    创新点：结合多尺度空洞深度卷积的空间注意力和全局通道注意力。
    专门针对医学图像中病灶尺度多变、边界模糊的问题设计。
    """
    ...
    def __init__(self, dim, reduction=8):
        ...
        # 2. 多尺度空间注意力分支 (Multi-Scale Spatial Branch)
        # 使用深度可分离卷积 + 不同扩张率获取多尺度感受野
        self.spatial_conv1 = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim)  # 局部
        self.spatial_conv2 = nn.Conv2d(dim, dim, kernel_size=3, padding=2, dilation=2, groups=dim)  # 中等
        self.spatial_conv3 = nn.Conv2d(dim, dim, kernel_size=3, padding=4, dilation=4, groups=dim)  # 全局
