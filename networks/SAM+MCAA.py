...
# ==========================================
# Modified Block Module (without Adapters)
# ==========================================
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # Adapter application removed: attn_output = attn_output + self.attn_adapter(attn_output)
        x = h + attn_output  # Standard residual connection for attention block

        h = x
        x = self.ffn_norm(x)
        mlp_output = self.ffn(x)
        # Adapter application removed: mlp_output = mlp_output + self.ffn_adapter(mlp_output)
