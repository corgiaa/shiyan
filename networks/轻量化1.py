...
class LightweightTransUNetDecoder(nn.Module):
    ...
    def __init__(self, encoder_channels, decoder_hidden_dim=128, out_channels=128):
        ...
        # 2. 上采样路径
        # 使用 SimpleUpBlock 替换 LGBU
        self.up_blocks = nn.ModuleList([
            # 第一次上采样 (例如 14x14 -> 28x28)
            SimpleUpBlock(decoder_hidden_dim, decoder_hidden_dim // 2),

            # 第二次上采样 (例如 28x28 -> 56x56)
            SimpleUpBlock(decoder_hidden_dim // 2, decoder_hidden_dim // 4)
        ])
        ...
        # 3. 添加 Dropout2d 层
        self.dropout = nn.Dropout2d(0.4) # 新增的 Dropout 层
    ...
    def forward(self, features):
        ...
        # 3. 逐步上采样 (使用 SimpleUpBlock)
        for up_block in self.up_blocks:
            x = up_block(x)

        # 4. 应用 Dropout
        x = self.dropout(x) # 在最终输出卷积前应用 Dropout
...
class VisionTransformer(nn.Module):
    ...
    使用了 timm 的 ViT-Tiny 作为骨干，并结合了带有 DDFE 和标准上采样块的解码器。
    """
    ...
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        logger.info("Initialized with Innovative Module: DualDomainFeatureEnhancer and Dropout2d(0.4)") # 更新日志信息
    ...
    def forward(self, x):
        ...
        # 2. Decoder Forward (包含 DDFE 和 Dropout)
