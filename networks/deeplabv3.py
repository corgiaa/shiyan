...
# 保留原始的ViT配置导入，尽管DeepLabV3+结构不会使用其大部分参数
...
class VisionTransformer(nn.Module):
    """
    此类别已重构为实现 segmentation_models_pytorch 的 DeepLabV3+ 架构，
    ...
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        # DeepLabV3+ 的中间输出通道数，这将作为SegmentationHead的输入通道数。
        # 通常是 DeepLabV3+ 解码器头的输出通道数，例如256。
        unet_output_channels = 256 # 命名保留，但现在是DeepLabV3+的中间输出通道

        # --- segmentation_models_pytorch DeepLabV3+ 参数配置 ---
        ...
        # 解码器通道数。DeepLabV3+ 的解码器通常有一个固定的通道数，例如256。
        # 这个参数控制 ASPP 模块和解码器输出的特征图通道数。
        ...
        self.unet_model = smp.DeepLabV3Plus(
            encoder_name=encoder_name,
            encoder_weights=encoder_weights,
            in_channels=in_channels,
            classes=unet_output_channels,  # DeepLabV3+ 的中间输出通道数，由SegmentationHead进一步处理
        ...
        self.segmentation_head = SegmentationHead(
            in_channels=unet_output_channels,  # 接收DeepLabV3+的中间特征图
    ...
    def forward(self, x):
        ...
        # 通过 segmentation_models_pytorch DeepLabV3+ 提取特征
    ...
    def load_from(self, weights):
        logger.warning(
            "The 'load_from' method is specific to Vision Transformer weights and is not applicable to the DeepLabV3+ architecture. Skipping weight loading.")
        pass
