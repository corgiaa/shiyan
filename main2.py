...
# ----------------------------------------------------------------------
# --- 新增：DiceLoss 和 CombinedLoss ---
# ----------------------------------------------------------------------

class DiceLoss(nn.Module):
    """
    计算Dice Loss。对于二分类分割 (n_classes=2)，主要关注前景类别 (class 1)。
    """
    ...
    def _dice_coefficient(self, score, target):
        """
        计算单个类别的Dice系数。
        Args:
            score (torch.Tensor): 预测的类别概率 (N, H, W)。
            target (torch.Tensor): 真实二值掩码 (N, H, W)，值为0或1。
        Returns:
            torch.Tensor: Dice系数。
        """
    ...
    def forward(self, inputs, target, softmax=False):
        ...
        计算Dice Loss。
        Args:
            inputs (torch.Tensor): 模型的原始logits (N, C, H, W) 或概率 (N, C, H, W)。
            target (torch.Tensor): 真实标签的类别索引 (N, H, W)，值为0或1。
            softmax (bool): 是否对输入应用softmax。
        Returns:
        ...
        # 对于二分类分割 (n_classes=2，例如息肉分割)，我们通常计算前景类别 (class 1) 的Dice。
        # inputs[:, 1] 提取前景类别的概率。
        # target.float() 将真实标签转换为浮点型，以便进行元素级乘法。

        if self.n_classes == 2:
            # 专注于前景 (class 1)
            dice_coeff = self._dice_coefficient(inputs[:, 1], target.float())
            loss = 1.0 - dice_coeff
        else:
            # 对于多分类分割 (n_classes > 2)，此DiceLoss实现需要扩展
            # 以处理one-hot编码的标签，并可能对所有相关类别求Dice Loss的平均值。
            # 对于当前的息肉分割场景，n_classes=2 是主要用例。
            raise NotImplementedError(
                "Multi-class DiceLoss (n_classes > 2) is not implemented in this version. "
                "请根据需要分割多个前景类别的情况进行适配。"
            )
        return loss
...
class CombinedLoss(nn.Module):
    """
    结合了交叉熵损失 (Cross-Entropy Loss) 和 Dice 损失 (Dice Loss) 用于分割任务。
    Args:
        n_classes (int): 类别数量。
    ...
    def __init__(self, n_classes, ce_weight=0.5, dice_weight=0.5):
        ...
        self.ce_loss = nn.CrossEntropyLoss()  # 处理多分类像素级分类
        self.dice_loss = DiceLoss(n_classes)  # 针对n_classes=2的前景Dice Loss
    ...
    def forward(self, outputs, targets):
        ...
        计算组合损失。
        Args:
            outputs (torch.Tensor): 模型输出的原始logits (N, C, H, W)。
            targets (torch.Tensor): 真实标签的类别索引 (N, H, W)，值为0或1。
        Returns:
            torch.Tensor: 交叉熵损失和Dice损失的加权和。
        """
        # 交叉熵损失期望logits和长整型标签
        ce_loss_val = self.ce_loss(outputs, targets.long())

        # Dice损失期望logits (内部会应用softmax) 和浮点型标签
        dice_loss_val = self.dice_loss(outputs, targets, softmax=True)
...
def plot_segmentation_results(image, gt_mask, pred_mask, foreground_probs_map, case_name, save_path):
    """
    绘制单张图像的输入、真值、预测、前景概率图和叠加结果 (5图)
    """
    fig, axes = plt.subplots(1, 5, figsize=(20, 4))  # 增加一个子图
    ...
    # 4. 前景概率图 (新增)
    # 使用一个合适的colormap，例如 'viridis' 或 'magma'
    im = axes[3].imshow(foreground_probs_map, cmap='viridis', vmin=0, vmax=1)
    axes[3].set_title("4. Foreground Probability")
    axes[3].axis('off')
    # 添加颜色条
    fig.colorbar(im, ax=axes[3], orientation='vertical', fraction=0.046, pad=0.04)

    # 5. 叠加图 (使用红色表示预测，绿色表示真值)
    # 创建一个叠加图像
    ...
    axes[4].set_title("5. Overlay (R: Pred, G: GT)")
    axes[4].axis('off')
...
def custom_test_single_volume(image_batch, label_batch, net, classes, patch_size, test_save_path=None, case=None,
                              z_spacing=1, visualize_sample=False, prediction_threshold=0.5):  # 新增 prediction_threshold
    """
    ...
    try:
        ...
        with torch.no_grad():
            ...
            # 假设输出是 (1, C, H, W)
            probabilities = torch.softmax(outputs, dim=1)  # 获取所有类别的概率

            # 提取前景（Class 1）的概率图
            # 注意：classes=2时，class 0是背景，class 1是前景
            foreground_probs_map = probabilities[:, 1, :, :].squeeze(0).cpu().numpy()

            # 根据阈值进行二值化预测
            prediction = (foreground_probs_map > prediction_threshold).astype(np.uint8)
        ...
        for class_idx in range(1, classes):
            ...
            if visualize_sample and class_idx == 1:
                plot_segmentation_results(image_resized, label_class, pred_class, foreground_probs_map, case,
                                          test_save_path)
...
parser.add_argument('--prediction_threshold', type=float, default=0.5,
                    help='Threshold for binarizing foreground probabilities during testing.')  # 新增参数
...
def create_single_dataset_split(dataset_folder_name, dataset_full_path, dataset_list_dir, split_ratio=0.8):
    ...
    logging.info(
        f"Creating {split_ratio * 100:.0f}/{(1 - split_ratio) * 100:.0f} split for dataset: {dataset_folder_name}")
            ...
            for i_batch, sampled_batch in tqdm(enumerate(testloader), desc=f"Testing {args.dataset} Epoch {epoch}",
                ...
                try:
                    metrics_i = custom_test_single_volume(
                        ...
                        prediction_threshold=args.prediction_threshold  # 传入阈值
                    )
...
def modified_trainer_synapse(args, model, snapshot_path):
    ...
    **FIXED: Increased num_workers for faster data loading.**
    ...
    model.train()
    # --- 关键修改：使用 CombinedLoss 替代 CrossEntropyLoss ---
    criterion = CombinedLoss(n_classes=num_classes, ce_weight=0.5, dice_weight=0.5)
    # -----------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    for current_dataset_name in POLYP_DATASETS:
        ...
        if not os.path.isdir(actual_dataset_path):
            ...
            logging.error(
                "Please ensure the data is correctly placed in the 'data/data' directory relative to the script.")
        ...
        print(f"Prediction Threshold: {args.prediction_threshold}")  # 打印新增参数
        print(f"--------------------------------------------------")
