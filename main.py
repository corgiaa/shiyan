...
# ----------------------------------------------------------------------
# --- 数据集分割函数 (已修改：添加 TIF 支持，修正 ETIS 命名) ---
# ----------------------------------------------------------------------
...
def create_combined_split(data_configs, base_data_dir, list_dir, split_ratio=0.9):
    ...
    # Datasets for 100% test (注意：已修正 ETIS-LaribPolypDB)
    ...
    **FIXED: Increased num_workers for faster data loading.**
    ...
    for epoch_num in iterator:
        ...
        if (epoch_num + 1) % args.test_interval == 0:
            logging.info(f"Performing comprehensive test at epoch {epoch_num + 1}")
            # 使用 args.dataset (即 'Polyp_Combined_Cross_Validation') 进行推理
...
if __name__ == "__main__":
    ...
    # 添加组合数据集的配置
    # 注意: args.root_path 必须是所有子数据集的父目录: F:\danzi10\4.5w\daima\TransUNet-main\data\data
    args.root_path = os.path.abspath(BASE_DATA_DIR_RELATIVE)
    args.list_dir = os.path.abspath(os.path.join('./lists', 'lists_Polyp_Combined'))
    ...
    # 使用修改后的训练器
    print("Starting training with comprehensive evaluation metrics and visualizations...")
